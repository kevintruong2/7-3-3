...
function generateMap(map_json) {
    ...
    paths.append("svg:text")
        ...
        .on("click", function (d, i) {
            ...
            var g = d3.select(this.parentNode)._groups[0];
            var id = g[0].childNodes[0].id;
            mapClicked(g[0].childNodes[0], svg);

        })
}

function mapClicked(obj, svg) {
    ...
    mapMarker.attr("stroke", "gold").attr("stroke-width", "3");
